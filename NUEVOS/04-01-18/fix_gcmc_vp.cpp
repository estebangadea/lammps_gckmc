...
FixGCMCVp::FixGCMCVp(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 16) error->all(FLERR,"Illegal fix gcmc command");
  ...
  // required args
// fix StWMC water gcmc/vp 1 20 0 5 9999 $T ${Pin} 0.0 pair Stw pressure ${Pin}  Added by Yamila
// VaA=VbB
  nevery = force->inumeric(FLERR,arg[3]); // cada cuantos pasos MD
  nexchanges = force->inumeric(FLERR,arg[4]); // intentos de MC
  nmcmoves = force->inumeric(FLERR,arg[5]); // movimientos
  ngcmc_type = force->inumeric(FLERR,arg[6]); // tipo de atomos a cambiar
  seed = force->inumeric(FLERR,arg[7]); // seed
  reservoir_temperature = force->numeric(FLERR,arg[8]);  //T
  chemical_potential = force->numeric(FLERR,arg[9]); //Pv
  displace = force->numeric(FLERR,arg[10]); //desp
  typeR=force->inumeric(FLERR,arg[11]); //reactivo
  typeP=force->inumeric(FLERR,arg[12]); //producto
  steqR=force->inumeric(FLERR,arg[13]); //coef estequio reactivo reactivo
  steqP=force->inumeric(FLERR,arg[14]); //coef estequio producto reactivo
  neveryrx=force->inumeric(FLERR,arg[15]); // cada cuantos pasos MD intento rx
  nrx=force->inumeric(FLERR,arg[16]); // intentos de rx
  printf("jojojo feliz 2018 \n");
  ...
  if(typeR<=0)  error->all(FLERR,"Illegal reactive type gcmc command");
  if(typeP<=0)  error->all(FLERR,"Illegal product type gcmc command");
  if(steqR!=1)  error->all(FLERR,"solo estequio 1 a 1 ...");
   if(steqR!=1)  error->all(FLERR,"solo estequio 1 a 1 ...");
  if (neveryrx < 0) error->all(FLERR,"Illegal nevertyrx fix gcmc command");
  if (nrx < 0) error->all(FLERR,"Illegal nrx fix gcmc command");
...
//  options(narg-11,&arg[11]);  commented by Yamila
    options(narg-17,&arg[17]); //added by Yamila
  ...
  ncycles = nexchanges + nmcmoves; //commented by Yamila
    ncycles = nexchanges + nmcmoves + nrx;
  ...
  nfoward_attempts =  0.0;
  nfoward_successes = 0.0;
  nreverse_attempts =  0.0;
  nreverse_successes = 0.0;
  
    energyout=0.0;  // Matias

  gcmc_nmax = 0;
  local_gas_list = NULL;
  local_reactive_list = NULL;
  local_product_list = NULL;
...
FixGCMCVp::~FixGCMCVp()
{
  ...
  memory->destroy(local_reactive_list); //added by Yamila
  memory->destroy(local_product_list); //added by Yamila
    ...
    if (mode == MOLECULE) {
      ...
      for (int i = 0; i < ncycles; i++) {
        int random_int_fraction =
          static_cast<int>(random_equal->uniform()*ncycles) + 1;
/*if (comm->me==0){
     printf("random, nmcmoves,nexchange, %d,%d,%d\n",random_int_fraction,nmcmoves,nexchanges);   
}*/
     if (random_int_fraction <= nmcmoves) {
          attempt_atomic_translation_full();
        } else { //coomented by Yamila
         } if ((random_int_fraction > nmcmoves) &&(random_int_fraction <= nmcmoves+nexchanges)) {  //added by Yamila
          if (random_equal->uniform() < 0.5) attempt_atomic_deletion_full();
          else attempt_atomic_insertion_full();
        }
         if (random_int_fraction  > nmcmoves+nexchanges ) {  //addded by Yamila
           if (random_equal->uniform() < 0.5) attempt_atomic_foward_full();
          else attempt_atomic_reverse_full();
      }
      }
    }
     ...
     if (comm->me==0){
      ...
      for (int i = 0; i < ncycles; i++) {
       ...
       if (random_int_fraction <= nmcmoves) {
        ...
        } //else { commented by Yamila
        if ((random_int_fraction > nmcmoves) &&(random_int_fraction <= nmcmoves+nexchanges)) { 
            double tmp_debug = -2;
      ...
      if (random_int_fraction  > nmcmoves+nexchanges ) {  //addded by Yamila
          if (random_equal->uniform() < 0.5) {
           attempt_atomic_foward_full();
           printf("voy a foward\n");    
        }else{ attempt_atomic_reverse_full();
           printf("voy a reverse\n");
    }
      }
...
/* ----------------------------------------------------------------------
 * ------------------------------------------------------------------------- */
void FixGCMCVp::attempt_atomic_foward_full()
{
   nfoward_attempts += 1.0;
   if (nr == 0) return;
   int i = pick_random_reactive_atom();
   int success = 0;
    ...
    double diff_energy;
 if (i >= 0) {
      if (atom->type[i] != typeR) {
          error->all(FLERR,"Exit type R is not correct");
      }

 if (pairflag == 0) {
     pre_energy = energy(i,typeR,-1,atom->x[i]);
      post_energy =  energy(i,typeP,-1,atom->x[i]);
      diff_energy=post_energy-pre_energy;
 ...
 if (random_unequal->uniform() <
       exp(-beta*diff_energy)*(nr)/((np+steqP))) {
       atom->type[i] = typeP;               
      success = 1;
   }
  }
...
int success_all = 0;
 ...
 if (success_all) {
    ...
    update_reactive_atoms_list();
     update_product_atoms_list();
    nfoward_successes += 1.0;
}
/* ----------------------------------------------------------------------
 *  * ------------------------------------------------------------------------- */
void FixGCMCVp::attempt_atomic_reverse_full()
{
   nreverse_attempts += 1.0;
   if (np == 0) return;
   ...
   int success = 0;
    ...
    double diff_energy;
 if (i >= 0) {
      if (atom->type[i] != typeP) {
          error->all(FLERR,"Exit type P is not correct");
      }
 if (pairflag == 0) {
      pre_energy = energy(i,typeP,-1,atom->x[i]);
      post_energy =  energy(i,typeR,-1,atom->x[i]);
      diff_energy=post_energy-pre_energy;
 ...
 if (random_unequal->uniform() <
       exp(-beta*diff_energy)*(np)/(nr+steqR)) {
       atom->type[i] = typeR;
      success = 1;
    }
  }
...
int FixGCMCVp::pick_random_reactive_atom()
{
  int i = -1;
  int iwhichglobal = static_cast<int> (nr*random_equal->uniform());
  if ((iwhichglobal >= nr_before) &&
      (iwhichglobal < nr_before + nr_local)) {
    int iwhichlocal = iwhichglobal - nr_before;
    i = local_reactive_list[iwhichlocal];
  }
...
int FixGCMCVp::pick_random_product_atom()
{
  int i = -1;
  int iwhichglobal = static_cast<int> (np*random_equal->uniform());
  if ((iwhichglobal >= np_before) &&
      (iwhichglobal < np_before + np_local)) {
    int iwhichlocal = iwhichglobal - np_before;
    i = local_product_list[iwhichlocal];
  }
...
/* ----------------------------------------------------------------------
 *    update the list of reactive atoms added by YAMILA
 *    ------------------------------------------------------------------------- */
void FixGCMCVp::update_reactive_atoms_list()
{
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  local_reactive_list = new int[nlocal];
 tagint *molecule = atom->molecule;
  double **x = atom->x;
  if (nlocal > gcmc_nmax) {
    memory->sfree(local_reactive_list);
    gcmc_nmax = atom->nmax;
    local_reactive_list = (int *) memory->smalloc(gcmc_nmax*sizeof(int),
     "GCMC:local_reactive_list");
  }
  ...
  nr_local = 0;
    int *type = atom->type; // added by Jibao
     ...
     for (int i = 0; i < nlocal; i++) {
        if ((mask[i] & groupbit) && (type[i] == typeR)){ //commented by YAmila
        local_reactive_list[nr_local] = i;
        nr_local++;
      }
  }
printf("nr local is,%d\n",nr_local);
  MPI_Allreduce(&nr_local,&nr,1,MPI_INT,MPI_SUM,world);
  MPI_Scan(&nr_local,&nr_before,1,MPI_INT,MPI_SUM,world);
  nr_before -= nr_local;
}

/* ----------------------------------------------------------------------
 *    update the list of product atoms added by Yamila
 *    ------------------------------------------------------------------------- */

void FixGCMCVp::update_product_atoms_list()
{
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  tagint *molecule = atom->molecule;
  double **x = atom->x;
   local_product_list = new int[nlocal];
  if (nlocal > gcmc_nmax) {
    memory->sfree(local_product_list);
    gcmc_nmax = atom->nmax;
    local_product_list = (int *) memory->smalloc(gcmc_nmax*sizeof(int),
     "GCMC:local_product_list");
  }
  ...
  np_local = 0;
    ...
    int *type = atom->type; // added by Jibao
        for (int i = 0; i < nlocal; i++) {
        if ((mask[i] & groupbit) && (type[i] == typeP)){
        local_product_list[np_local] = i;
         np_local++;
      }
  }
   printf("np_local is %d \n",np_local);
  MPI_Allreduce(&np_local,&np,1,MPI_INT,MPI_SUM,world);
  MPI_Scan(&np_local,&np_before,1,MPI_INT,MPI_SUM,world);
  np_before -= np_local;
}
...
double FixGCMCVp::compute_vector(int n)
{
 ...
 if (n == 9) return nfoward_attempts; 
 if (n == 10) return nfoward_successes;
 if (n == 11) return nreverse_attempts;  
 if (n == 12) return nreverse_successes; 
 return 0.0;
}
