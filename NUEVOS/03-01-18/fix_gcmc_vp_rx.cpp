...
FixGCMCVp::FixGCMCVp(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 50) error->all(FLERR,"Illegal fix gcmc command");
...
// VaA=VbB
  ...
  if(steqR!=0)  error->all(FLERR,"solo estequio 1 a 1 ...");
   if(steqR!=0)  error->all(FLERR,"solo estequio 1 a 1 ...");
...
void FixGCMCVp::update_reactive_atoms_list()
{
  ...
  nr_local = 0;
    ...
    int *type = atom->type; // added by Jibao
        for (int i = 0; i < nlocal; i++) {
        if ((mask[i] & groupbit) && (type[i] == typeR)){
