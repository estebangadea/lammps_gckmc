      ...
      for (int i = 0; i < ncycles; i++) {
        ...
        if (random_int_fraction <= nmcmoves) {
          ...
          if (random_equal->uniform() < 0.5) attempt_atomic_deletion_full();
              ...
              //printf("comm->me = %d: Before attempt_atomic_insertion_full in FixGCMC::pre_exchange()\n",comm->me);
...
void FixGCMC::attempt_atomic_deletion_full()
{
  ...
  double energy_after = energy_full();
      ...
      //added by Jibao; only for debugging: output the energy difference before and after the deletion/insertion of atoms calculated by Jibao's energy() function, and compare the value computed with the full_energy method
     
    //printf("comm->me = %d, deletion_full: energy_before-energy_after = %f\n",comm->me,energy_before-energy_after); //added by Jibao
    /*
    if (flag) {
        double deletion_energy = energy(i,ngcmc_type,-1,atom->x[i]);
        printf("comm->me = %d, i = %d, deletion_energy=energy() = %f\n\n",comm->me,i,deletion_energy);    // added by Jibao
    }
    */
  ...
  update_gas_atoms_list();
    ...
    //printf("comm->me = %d: End of FixGCMC::attempt_atomic_deletion_full()\n",comm->me);
}
...
{
    //printf("comm->me = %d: Beginning of FixGCMC::attempt_atomic_insertion_full()()\n",comm->me);
  double lamda[3];
  ninsertion_attempts += 1.0;
  ...
  if (triclinic == 0) {
    ...
    //int tmp_jibao = -1;  // added by Jibao; only for debugging
  if (proc_flag) {
    atom->avec->create_atom(ngcmc_type,coord);
    int m = atom->nlocal - 1;
    ...
    modify->create_attribute(m);
      ...
      //tmp_jibao = m;    // added by Jibao; only for debugging
  ...
  comm->borders();
    ...
    /*
    double insertion_energy = energy(tmp_jibao,ngcmc_type,-1,coord);  //added by Jibao; this line has to be after atom->nghost = 0 to avoid the ngost+nlocal being larger than the available list for nmap, map_substyle, ...; it also has to be before if (force->kspace) force->kspace->qsum_qsq(), otherwise, the return value of energy() may be wrong. commented by Jibao
    
    printf("comm->me = %d: m = %d, insertion_energy=energy() = %f\n",comm->me,tmp_jibao,insertion_energy);    //added by Jibao; only for debugging: output the energy difference before and after the deletion/insertion of atoms calculated by Jibao's energy() function, and compare the value computed with the full_energy method
    */
    
  if (force->kspace) force->kspace->qsum_qsq();
  double energy_after = energy_full();

  //printf("comm->me = %d, insertion_full: energy_before-energy_after = %f\n\n",comm->me,energy_before-energy_after); //added by Jibao
...
/*     // added by Jibao; just for debug: calculate the energy difference before and after the deletion/insertion of atoms, and compare the value computed with the full_energy method
 */
