...
FixStyle(labelchange,FixLabChange)

#else

#ifndef LMP_FIX_LABCHANGE_H
#define LMP_FIX_LABCHANGE_H
...
class FixLabChange : public Fix {
 public:
  FixLabChange(class LAMMPS *, int, char **);
  ~FixLabChange();
  int setmask();
  void init();
  void pre_exchange();
  ...
  void attempt_label_exchange(int); //Esteban
 ...
 private:
  ...
  int initial_type, final_type; //Esteban
