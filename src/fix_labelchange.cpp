...
FixLabChange::FixLabChange(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  initial_type = force->inumeric(FLERR,arg[3]);
  final_type = force->inumeric(FLERR,arg[4]);
  relation = force->numeric(FLERR,arg[5]);
  seed = force->inumeric(FLERR,arg[6]);
  
 //Esteban: initial_type, final_type, E, region, nreactions
  ...
  local_prod_list = NULL;  //Esteban
   ...
   // if (comm->me == 0) printf("End of FixLabChange::FixLabChange()\n");
}
...
void FixLabChange::options(int narg, char **arg)
{
  ...
  if (strcmp(arg[iarg],"mol") == 0) {
	...
	grouptypestrings = (char**)
     ...
     // if (comm->me == 0) printf("End of FixLabChange::options()\n");
}

/* ---------------------------------------------------------------------- */

FixLabChange::~FixLabChange()
{
   // printf("FixLabChange()");
...
void FixLabChange::init()
{
   // if (comm->me == 0) printf("Begins: FixLabChange::init()\n");
  ...
  if (!full_flag) {
    if ((force->kspace) ||
      ...
      full_flag = false;
        ...
        //if (comm->me == 0) printf("Begins: inside if (!full_flag){}: FixLabChange::init()\n");
    ...
    //if (comm->me == 0) printf("Begins 2: FixLabChange::init()\n");
  ...
  if (mode == ATOM) {
    if (final_type <= 0 || final_type > atom->ntypes)
      error->all(FLERR,"Invalid atom type in fix gcmc command");
    if (initial_type <= 0 || initial_type > atom->ntypes)
      error->all(FLERR,"Invalid atom type in fix gcmc command");
  }
    
    //if (comm->me == 0) printf("Begins 3: FixLabChange::init()\n");
  ...
  if ((mode == ATOM) && atom->molecule_flag) {
      /*
      if (comm->me == 0) {
          printf("Inside if ((mode == ATOM)): FixLabChange::init()\n");
    ...
    for (int i = 0; i < atom->nlocal; i++)
      ...
      //if (comm->me == 0) printf("Inside if ((mode == ATOM)) 2: FixLabChange::init()\n");
  ...
  if (mode == MOLECULE) {
    ...
    //if (comm->me == 0) printf("Begins 5: FixLabChange::init()\n");
  ...
  // its molecule template must be same as this one
    ...
    //if (comm->me == 0) printf("Begins 6: FixLabChange::init()\n");
  ...
  if (shakeflag) {
    ...
    //if (comm->me == 0) printf("Begins 7: FixLabChange::init()\n");
  ...
  // create a new group for interaction exclusions
    ...
    //if (comm->me == 0) printf("Before 'create a new group for interaction exclusions': FixLabChange::init()\n");
  ...
  if (atom->firstgroup >= 0) {
    ...
    //if (comm->me == 0) printf("Before 'compute beta, lambda, sigma, and the zz factor': FixLabChange::init()\n");
...
//    }
  ...
  if (ngrouptypes > 0) {
    ...
    //printf("End of FixLabChange::init()\n");
...
void FixLabChange::pre_exchange()
{
  // just return if should not be called on this timestep
 //if (comm->me == 0) printf("Begin of FixLabChange::pre_exchange()\n");
  ...
  next_reneighbor = update->ntimestep+1;
 //if (comm->me == 0) printf("End of FixLabChange::pre_exchange()\n");
}

void FixLabChange::attempt_label_exchange(int i)
{
  ...
  int j = local_react_list[ilocal];
    ...
    double **x = atom->x;
    int *type = atom->type;
    //double energy_before = energy(i, initial_type,-1,x[i]);
    //double energy_after = energy(i, final_type,-1,x[i]);
...
int FixLabChange::pick_random_product_atom()
{
  int i = -1;
...
void FixLabChange::update_gas_atoms_list()
{
//printf("Begin of FixLabChange::update_gas_atoms_list()\n");
  ...
  ngas_local = 0;
 //printf("End of FixLabChange::update_gas_atoms_list()\n");
...
void FixLabChange::update_reactive_atoms_list()
{
 //if (comm->me == 0) printf("Begin of FixLabChange::update_reactive_atoms_list()\n");
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  tagint *molecule = atom->molecule;
  double **x = atom->x;
...
//  }
  ...
  if (regionflag) {
    ...
    if (mode == MOLECULE) {
      ...
      for (int i = 0; i < nlocal; i++) {
          if ((mask[i] & groupbit) && (type[i] == initial_type)) {  // Modified by Jibao
...
void FixLabChange::update_product_atoms_list()
{
 //if (comm->me == 0) printf("Begin of FixLabChange::update_product_atoms_list()\n");
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  tagint *molecule = atom->molecule;
  double **x = atom->x;
...
//  }
  ...
  if (regionflag) {
    ...
    if (mode == MOLECULE) {
      ...
      for (int i = 0; i < nlocal; i++) {
          if ((mask[i] & groupbit) && (type[i] == final_type)) {  // Modified by Jibao
        //if (mask[i] & groupbit) { // commented out by Jibao
          if (domain->regions[iregion]->match(x[i][0],x[i][1],x[i][2]) == 1) {
...
double FixLabChange::compute_vector(int n)
{
...
double FixLabChange::memory_usage()
{
...
void FixLabChange::write_restart(FILE *fp)
{
  int n = 0;
  double list[4];
...
void FixLabChange::restart(char *buf)
{
  int n = 0;
